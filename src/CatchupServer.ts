...
import { CatchupMessage, eventMessages, mediaTuple } from "./packet_definitions";
...
export class CatchupServer {
	...
	constructor(master: Master) {
		...
		this.socket = dgram.createSocket("udp4",(msg, rinfo)=>this.receiver(msg, rinfo.port, rinfo.address));
        this.socket.bind()
    ...
    private receiver(msg: Buffer, port: number, address: IP) {
        ...
        if (msg.toString() === "getTimes"){
            ...
            this.socket.send(this.eventsMessage(), port, address)
